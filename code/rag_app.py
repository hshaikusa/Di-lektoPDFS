...
Important: Base your entire response solely on the information provided in the context. Do not include any external knowledge or assumptions not present in the given text. If there are no relevant infomration found, simply respond with "Sorry!! There are relevant references found from the existing document(s)."
"""
...
def get_vector_collection() -> chromadb.Collection:
    ...
    # Creating embedding function: The below code snippet initializes OllamaEbeddingFunction 
    # - url specifies the endpoint for the embedding API.
    # - model_name specifies the model to use for generating embeddings, in this case, nomic-embed-text:latest.
    ...
    # path specifies the directory where the persistent data will be stored (./demo-rag-chroma).

    chroma_client = chromadb.PersistentClient(path="./demo-rag-chroma")
    

    # The metadata setting {"hnsw:space": "cosine"} is used to configure the space in which the Hierarchical Navigable Small World (HNSW) algorithm operates.
...
with st.sidebar:
    ...
    if uploaded_file and process:
        ...
        st.write(normalize_uploaded_file_name)
        st.write(all_splits)
